 ...
 * centered in the array. Values inside the cutoff radius `r_cutoff` are set
 * to 1.0. Between `r_cutoff` and the normalized radius 1.0, the values smoothly
 ...
 * this radius, the kernel has a flat value of 1.0; outside, it decays smoothly
 * to 0.0.
 *
 * @return A 2D array of size `shape` containing the smooth disk-shaped kernel.
 *
