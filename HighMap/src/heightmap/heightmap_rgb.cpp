...
HeightMapRGB mix_heightmap_rgb_ryb(HeightMapRGB &rgb1,
                                   HeightMapRGB &rgb2,
                                   HeightMap    &t)
{
  HeightMapRGB rgb_out;
  ...
  auto lambda = [](Array &r_out,
                   ...
                   Array &t)
  {
    // RGB to RYB conversion
    Array rc1 = Array(r1.shape);
    Array yc1 = Array(r1.shape);
    Array bc1 = Array(r1.shape);
    convert_rgb_to_ryb(r1, g1, b1, rc1, yc1, bc1);

    Array rc2 = Array(r2.shape);
    Array yc2 = Array(r2.shape);
    Array bc2 = Array(r2.shape);
    convert_rgb_to_ryb(r2, g2, b2, rc2, yc2, bc2);

    // mix in RYB space
    Array rc = lerp(rc1, rc2, t);
    Array yc = lerp(yc1, yc2, t);
    Array bc = lerp(bc1, bc2, t);

    convert_ryb_to_rgb(rc, yc, bc, r_out, g_out, b_out);
  };

  // apply to the each rgb heightmaps
  size_t                         nthreads = rgb1.rgb[0].get_ntiles();
  std::vector<std::future<void>> futures(nthreads);

  for (decltype(futures)::size_type i = 0; i < nthreads; ++i)
    futures[i] = std::async(lambda,
                            std::ref(rgb_out.rgb[0].tiles[i]),
                            std::ref(rgb_out.rgb[1].tiles[i]),
                            std::ref(rgb_out.rgb[2].tiles[i]),
                            std::ref(rgb1.rgb[0].tiles[i]),
                            std::ref(rgb1.rgb[1].tiles[i]),
                            std::ref(rgb1.rgb[2].tiles[i]),
                            std::ref(rgb2.rgb[0].tiles[i]),
                            std::ref(rgb2.rgb[1].tiles[i]),
                            std::ref(rgb2.rgb[2].tiles[i]),
                            std::ref(t.tiles[i]));
